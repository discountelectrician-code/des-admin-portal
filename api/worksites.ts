...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    try {
      const snapshot = await getDocs(collection(db, 'worksites'));
      const worksites = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      return res.status(200).json(worksites);
    } catch (error: any) {
      console.error('Worksite API Error (GET):', error);
